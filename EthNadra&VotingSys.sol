...
contract NicSystem{
  ...
  // like that you can create other things bank acount licenses etc just give your Id & Click
  function createDomicile(uint _cnicId) public view returns
    (uint,
    string memory ,
	  string memory ,
   // string memory  ,
    ...
    string memory )
  {
  return (Resitraion[_cnicId].nicId,
  Resitraion[_cnicId].Name,
  Resitraion[_cnicId].FatherName,
 // Resitraion[_cnicId].MotherName,
  Resitraion[_cnicId].gender,
  Resitraion[_cnicId].currentAdress,
  Resitraion[_cnicId].DateOfBirth,
  "\n congrates Your Domicile Is Created Successfly");
  }
